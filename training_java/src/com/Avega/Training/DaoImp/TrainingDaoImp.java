...
public class TrainingDaoImp implements TrainingDao{
	...
	SkillDao dao1 = new SkillDaoImp();
	List<Training> trainings = new ArrayList<>();
	List<Employee> employees = dao.getAllEmployee();
	List<Skill> skills = dao1.getAllSkill();
		...
		try {
			...
			while (rs.next()) {
				...
				for(Employee em : employees) {
					if(em.getEmployee_id().equalsIgnoreCase(rs.getString(6))) {
						e.setEmployee_id(em.getEmployee_id());
						e.setName(em.getName());
						e.setDoj(em.getDoj());
						e.setDesignation(em.getDesignation());
						e.setDepartment(em.getDepartment());
						e.setSkills(em.getSkills());
						break;
					}
				}
				for(Skill sk : skills) {
					if(sk.getSkill_id().equalsIgnoreCase(rs.getString(7))) {
						s.setSkill_id(sk.getSkill_id());
						s.setDescription(sk.getDescription());
						break;
					}
				}
				trainings.add(new Training(rs.getString(1),rs.getString(2),rs.getDate(3),rs.getDate(4),rs.getString(5),e,s));
			}
		...
		try {
			...
			while (rs.next()) {
				...
				if(rs.getString(1).equalsIgnoreCase(id)) {
					training = new Training(rs.getString(1),rs.getString(2),rs.getDate(3),rs.getDate(4),rs.getString(5),e,s);
				}
			}
	...
	public boolean addTraining(Training training) {
		...
		try {
			...
			ps.setString(6, training.getTrainer().getEmployee_id());
			ps.setString(7, training.getSkill().getSkill_id());
	...
	public boolean updateTraining(Training training) {
		...
		try {
					...
					ps.setString(5, training.getTrainer().getEmployee_id());
					ps.setString(6, training.getSkill().getSkill_id());
